    ...
    login: (req, res) => {
      ...
      loginFill: async (req, res) => {
        ...
        try {
            ...
            // Extract hashed password and role from database
            const hashedPassword = data[0].password;
            const userRole = data[0].role; // Assuming role is saved in the database
    
            // Compare the password
            const result = await bcrypt.compare(password, hashedPassword);
    
            if (result) {
                // Check if the role from the database matches the role in the request
                if (role !== userRole) {
                    return res.json({
                        status: "Failed",
